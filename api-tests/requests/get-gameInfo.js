...
export function postResponse() {
  ...
  pm.test('player2 has a valid id', () => {
    ...
    pm.expect(player2.id).to.equal(player2Id);
  });

  pm.test('Turn has a player id', () => {
    pm.expect(json.turn).to.be.a('string').that.is.not.empty;
    pm.expect(json.turn).to.be.oneOf([player1Id, player2Id]);
  });

  pm.environment.set('player1Id', player1Id);
  pm.environment.set('player2Id', player2Id);
}